...
class Log {
    ...
    protected $_log_path;
    protected $_log_name;
    protected $_log_config_file = 'log_config.php';
    protected $_log_cut;
    ...
    public function __construct() {
        if (empty(self::$log_unique_id)) {
            if (isset($_SERVER['HTTP_X_APUS_LOGID64']) && !empty($_SERVER['HTTP_X_APUS_LOGID64']) && $this->log_id_from = 'Header'){
                if(is_numeric((string)$_SERVER['HTTP_X_APUS_LOGID64'])){
                    self::$log_unique_id = $_SERVER['HTTP_X_APUS_LOGID64'];
                    unset($_SERVER['HTTP_X_APUS_LOGID64']);
                }
            }
            if(empty(self::$log_unique_id)){
                if (isset($_REQUEST['log_id']) && !empty($_REQUEST['log_id']) && $this->log_id_from = 'Body') {
                    self::$log_unique_id = $_REQUEST['log_id'];
                    unset($_REQUEST['log_id']);
                } else {
                    self::$log_unique_id = $this->generate_log_id();
                }
            }
        }

        if (defined('ENVIRONMENT') AND file_exists(APPPATH . 'config/' . ENVIRONMENT . '/' . $this->_log_config_file)) {
            require_once(APPPATH . 'config/' . ENVIRONMENT . '/' . $this->_log_config_file);
        } else {
            require_once(APPPATH . 'config/' . $this->_log_config_file);
        }
        ...
        if (!is_dir($this->_log_path) OR !is_really_writable($this->_log_path)) {
            error_log('[' . date('Y-m-d H:i:s') . '] ' . 'dir cant write:' . $this->_log_path . PHP_EOL, 3, '/tmp/php_error.log');
            return FALSE;
        }
            ...
            switch ($value) {
                case 'file':
                    ...
                    if ('H' === strtoupper($this->_log_cut)) {
                        $handler->setFilenameFormat('{filename}.{date}', 'YmdH');
                    }
...
//                    $handler = new BufferHandler($handler);
        ...
        $this->write_log('DEBUG', 'Log Class Initialized, Id From: ' . $this->log_id_from);
    }
    ...
    public function write_log($level = 'error', $msg, $context = array(), $php_error = FALSE) {
        ...
        if ($this->config['introspection_processor'] && $this->_threshold > 3) {
        ...
        if ($this->_levels[$level] <= $this->_threshold) {
            switch ($level) {
                ...
                case 'ERROR':
                    ...
                    break;
                case 'WARNING':
                    $this->log->addWarning($msg, $context);
                    break;
                case 'NOTICE':
    ...
    private function generate_log_id(){
        $this->log_id_from = 'Generate';
        list($usec, $sec) = explode(' ', microtime());
        return $sec . substr($usec, 2, 6) . mt_rand(100, 999);
    }
